...
/*   Updated: 2021/03/31 20:59:05 by ngregori         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	validate_r(t_scene *settings, char **strs)
{
	...
	if (!strs[1] || !strs[2] || settings->res)
		error_and_exit_settings(settings, "Error with handling resolution");
	...
	if (resolution)
	{
		...
		if (resolution->x <= 0 || resolution->y <= 0)
			error_and_exit_settings(settings, "Only positive values for x and y are valid for resolution");
		else
			settings->res = resolution;
	}
	else
		error_and_exit_settings(settings, "Invalid string format when checking resolution values");
}
...
void	validate_floor_ceil(t_scene *settings, t_rgb **floor_or_ceil, char **strs)
{
	...
	if (!strs[1] || *floor_or_ceil)
		error_and_exit_settings(settings, "Error found when handling floor or ceiling values");
	tmp_strs = ft_split(strs[1], ',');
	if (!tmp_strs[0] || !tmp_strs[1] || !tmp_strs[2])
		error_and_exit_settings(settings, "Error found when handling floor or ceiling, 3 color values are needed");
	...
	if (tmp)
	{
		...
		*floor_or_ceil = tmp;
		...
		error_and_exit_settings(settings, "Invalid string format when checking floor/ceiling values");
...
void	validate_textures(t_scene *settings, char **path, char **strs)
{
	...
	if (!strs[1] || *path)
		error_and_exit_settings(settings, "Error found when handling path to textures");
