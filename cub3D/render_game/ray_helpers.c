...
/*   Updated: 2021/05/14 16:58:23 by ngregori         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		did_ray_hit_wall(t_game *game, t_ray *ray, t_intercect intercect, bool hrzt)
{
			...
			intercect.x > 0 && intercect.y > 0)
	{
		if (is_wall(intercect.x, intercect.y, game))
		{
			...
			{
				ray->w_hrzt_hit = true;
				ray->w_hrzt_x = intercect.x;
				ray->w_hrzt_y = intercect.y;
			}
			else
			{
				ray->w_vrtc_hit = true;
				ray->w_vrtc_x = intercect.x;
				ray->w_vrtc_y = intercect.y;
			}
			return ;
		}
		else
...
void		get_horizontal_intercection(t_game *game, t_ray *ray)
{
	...
	intercect.x = ray->line.x + (intercect.y - ray->line.y) / tan(ray->line.dir);
	intercect.y_step = game->minimap_tile.size;
	if (ray->facing_up)
		intercect.y_step *= -1;
	intercect.x_step = game->minimap_tile.size / tan(ray->line.dir);
...
void		get_vertical_intercection(t_game *game, t_ray *ray)
{
	...
	intercect.y = ray->line.y + (intercect.x - ray->line.x) * tan(ray->line.dir);
	intercect.x_step = game->minimap_tile.size;
	if (!ray->facing_right)
		intercect.x_step *= -1;
	intercect.y_step = game->minimap_tile.size * tan(ray->line.dir);
